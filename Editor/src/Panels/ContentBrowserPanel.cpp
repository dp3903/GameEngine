	...
	void ContentBrowserPanel::OnImGuiRender()
	{
		...
		for (auto& directoryEntry : std::filesystem::directory_iterator(m_CurrentDirectory))
		{
			...
			else if (ext == ".lua")
			{
				...
				{
					const wchar_t* itemPath = relativePath.c_str();
					ImGui::SetDragDropPayload("CONTENT_BROWSER_ITEM_SCRIPT", itemPath, (wcslen(itemPath) + 1) * sizeof(wchar_t));
					ImGui::EndDragDropSource();
				}
			}
			else if (ext == ".ttf")
			{
				...
				{
					const wchar_t* itemPath = relativePath.c_str();
					ImGui::SetDragDropPayload("CONTENT_BROWSER_ITEM_FONT", itemPath, (wcslen(itemPath) + 1) * sizeof(wchar_t));
					ImGui::EndDragDropSource();
				}
			}
			else if (ext == ".wav" || ext == ".mp3" || ext == ".flac")
			{
				...
				{
					const wchar_t* itemPath = relativePath.c_str();
					ImGui::SetDragDropPayload("CONTENT_BROWSER_ITEM_SOUND", itemPath, (wcslen(itemPath) + 1) * sizeof(wchar_t));
					ImGui::EndDragDropSource();
				}
			}
