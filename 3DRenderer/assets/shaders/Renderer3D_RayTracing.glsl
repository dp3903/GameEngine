...
// --- Binding 2: Cuboid Data ---
struct Cuboid {
    vec3 Position;
    float Roughness; // Packs perfectly with Position (12 + 4 = 16)
    
    vec3 Rotation;
    float Metallic;  // Packs perfectly with Rotation
    
    vec3 Scale;
    float Opacity;   // Packs perfectly with Scale
    
    vec3 Albedo;
    float IOR;       // Packs perfectly with Albedo
};

layout(std140, binding = 2) uniform CuboidData
{
    Cuboid u_Cuboids[10];
    int u_CuboidCount;
};
...
// Rodrigues' rotation formula to rotate a point p around an arbitrary axis by a certain angle
...
HitInfo TraceRay(vec3 rayOrigin, vec3 rayDir)
{
    ...
    for (int i = 0; i < u_SphereCount; i++)
    {
        ...
        if (discriminant > 0.0)
        {
            ...
            float tFront = t1 < 0.001 ? t2 : t1; // If t1 is behind the ray, use t2 instead
    ...
    // Check cuboids
    for (int i = 0; i < u_CuboidCount; i++)
    {
        vec3 XAxis = vec3(1.0, 0.0, 0.0);
        vec3 YAxis = vec3(0.0, 1.0, 0.0);
        vec3 ZAxis = vec3(0.0, 0.0, 1.0);

        // Apply X rotation to ALL axes
        YAxis = rotateAxis(YAxis, XAxis, u_Cuboids[i].Rotation.x);
        ZAxis = rotateAxis(ZAxis, XAxis, u_Cuboids[i].Rotation.x);

        // Apply Y rotation to ALL axes
        XAxis = rotateAxis(XAxis, YAxis, u_Cuboids[i].Rotation.y);
        ZAxis = rotateAxis(ZAxis, YAxis, u_Cuboids[i].Rotation.y);

        // Apply Z rotation to ALL axes
        XAxis = rotateAxis(XAxis, ZAxis, u_Cuboids[i].Rotation.z);
        YAxis = rotateAxis(YAxis, ZAxis, u_Cuboids[i].Rotation.z);

        XAxis = normalize(XAxis);
        YAxis = normalize(YAxis);
        ZAxis = normalize(ZAxis);

        // Eqn of a plane is (P - P0) dot N = D
        // We can rearrange to find the intersection of a ray with the 6 planes of the cuboid, and then check if the intersection point is within the bounds of the cuboid's scale on the other 2 axes.
        // t = (-rayOrigin dot N +- D) / (rayDir dot N). if rayDir dot N is 0, the ray is parallel to the plane and we can skip it.
        if(abs(dot(rayDir, XAxis)) > 0.001)
        {
            float d1 = (-dot((rayOrigin - u_Cuboids[i].Position), XAxis) - u_Cuboids[i].Scale.x / 2) / dot(rayDir, XAxis);
            float d2 = (-dot((rayOrigin - u_Cuboids[i].Position), XAxis) + u_Cuboids[i].Scale.x / 2) / dot(rayDir, XAxis);
            float t = (d1 > 0 && (d1 < d2 || d2 < 0)) ? d1 : d2; // We want the +ve closer intersection
            if (t > 0.001 && t < closestT)
            {
                vec3 hitPoint = rayOrigin + rayDir * t;
                vec3 localHitPoint = hitPoint - u_Cuboids[i].Position;
                if (abs(dot(localHitPoint, YAxis)) <= u_Cuboids[i].Scale.y / 2.0 && abs(dot(localHitPoint, ZAxis)) <= u_Cuboids[i].Scale.z / 2.0)
                {
                    closestT = t;
                    hit.ObjectID = i + 100; // Cuboid IDs start at 100 to avoid clashing with sphere IDs
                    hit.Position = hitPoint;
                    hit.Normal = t==d1 ? -XAxis : XAxis; // Normal points against the surface
                }
            }
        }
        if(abs(dot(rayDir, YAxis)) > 0.001)
        {
            float d1 = (-dot((rayOrigin - u_Cuboids[i].Position), YAxis) - u_Cuboids[i].Scale.y / 2) / dot(rayDir, YAxis);
            float d2 = (-dot((rayOrigin - u_Cuboids[i].Position), YAxis) + u_Cuboids[i].Scale.y / 2) / dot(rayDir, YAxis);
            float t = (d1 > 0 && (d1 < d2 || d2 < 0)) ? d1 : d2; // We want the +ve closer intersection
            if (t > 0.001 && t < closestT)
            {
                vec3 hitPoint = rayOrigin + rayDir * t;
                vec3 localHitPoint = hitPoint - u_Cuboids[i].Position;
                if (abs(dot(localHitPoint, XAxis)) <= u_Cuboids[i].Scale.x / 2.0 && abs(dot(localHitPoint, ZAxis)) <= u_Cuboids[i].Scale.z / 2.0)
                {
                    closestT = t;
                    hit.ObjectID = i + 100; // Cuboid IDs start at 100 to avoid clashing with sphere IDs
                    hit.Position = hitPoint;
                    hit.Normal = t==d1 ? -YAxis : YAxis; // Normal points against the surface
                }
            }
        }
        if(abs(dot(rayDir, ZAxis)) > 0.001)
        {
            float d1 = (-dot((rayOrigin - u_Cuboids[i].Position), ZAxis) - u_Cuboids[i].Scale.z / 2) / dot(rayDir, ZAxis);
            float d2 = (-dot((rayOrigin - u_Cuboids[i].Position), ZAxis) + u_Cuboids[i].Scale.z / 2) / dot(rayDir, ZAxis);
            float t = (d1 > 0 && (d1 < d2 || d2 < 0)) ? d1 : d2; // We want the +ve closer intersection
            if (t > 0.001 && t < closestT)
            {
                vec3 hitPoint = rayOrigin + rayDir * t;
                vec3 localHitPoint = hitPoint - u_Cuboids[i].Position;
                if (abs(dot(localHitPoint, XAxis)) <= u_Cuboids[i].Scale.x / 2.0 && abs(dot(localHitPoint, YAxis)) <= u_Cuboids[i].Scale.y / 2.0)
                {
                    closestT = t;
                    hit.ObjectID = i + 100; // Cuboid IDs start at 100 to avoid clashing with sphere IDs
                    hit.Position = hitPoint;
                    hit.Normal = t==d1 ? -ZAxis : ZAxis; // Normal points against the surface
                }
            }
        }
...
vec3 CalculateLightTransmission(vec3 hitPoint, vec3 normal)
{
    ...
    for (int i = 0; i < u_SphereCount; i++)
    {
        ...
        if (discriminant > 0.0)
        {
            ...
            bool c1 = t1 > 0.001 && t1 < distToLight;
            bool c2 = t2 > 0.001 && t2 < distToLight;
            if (c1 || c2)
            {
                ...
                if (c1 && c2) {
                    lightMultiplier *= filterColor * (1.0 - alpha); // If the ray goes through the entire sphere, apply the filter twice for a stronger effect
                }
    ...
    for (int i = 0; i < u_CuboidCount; i++)
    {
        ...
        if(abs(dot(rayDir, XAxis)) > 0.001)
        {
            ...
            vec3 p1 = rayOrigin + rayDir * d1;
            vec3 p2 = rayOrigin + rayDir * d2;

            bool c1 = d1 > 0.001 && d1 < distToLight && abs(dot((p1 - u_Cuboids[i].Position), YAxis)) <= u_Cuboids[i].Scale.y / 2.0 && abs(dot((p1 - u_Cuboids[i].Position), ZAxis)) <= u_Cuboids[i].Scale.z / 2.0;
            bool c2 = d2 > 0.001 && d2 < distToLight && abs(dot((p2 - u_Cuboids[i].Position), YAxis)) <= u_Cuboids[i].Scale.y / 2.0 && abs(dot((p2 - u_Cuboids[i].Position), ZAxis)) <= u_Cuboids[i].Scale.z / 2.0;

            if (c1 || c2)
            {
                float alpha = u_Cuboids[i].Opacity; // Use the Opacity property to determine how much light is blocked
                ...
                // 2. Transparent objects tint the light and reduce its intensity
                // Mix between pure white (no tint) and the object's color based on alpha
                vec3 filterColor = mix(vec3(1.0), u_Cuboids[i].Albedo, alpha);
                
                // Multiply the remaining light by the filter, and reduce by (1 - alpha)
                lightMultiplier *= filterColor * (1.0 - alpha);
                if (c1 && c2) {
                    lightMultiplier *= filterColor * (1.0 - alpha); // If the ray goes through the entire cuboid, apply the filter twice for a stronger effect
                }
        ...
        if(abs(dot(rayDir, YAxis)) > 0.001)
        {
            ...
            bool c1 = d1 > 0.001 && d1 < distToLight && abs(dot((p1 - u_Cuboids[i].Position), XAxis)) <= u_Cuboids[i].Scale.x / 2.0 && abs(dot((p1 - u_Cuboids[i].Position), ZAxis)) <= u_Cuboids[i].Scale.z / 2.0;
            bool c2 = d2 > 0.001 && d2 < distToLight && abs(dot((p2 - u_Cuboids[i].Position), XAxis)) <= u_Cuboids[i].Scale.x / 2.0 && abs(dot((p2 - u_Cuboids[i].Position), ZAxis)) <= u_Cuboids[i].Scale.z / 2.0;
        ...
        if(abs(dot(rayDir, ZAxis)) > 0.001)
        {
            ...
            bool c1 = d1 > 0.001 && d1 < distToLight && abs(dot((p1 - u_Cuboids[i].Position), XAxis)) <= u_Cuboids[i].Scale.x / 2.0 && abs(dot((p1 - u_Cuboids[i].Position), YAxis)) <= u_Cuboids[i].Scale.y / 2.0;
            bool c2 = d2 > 0.001 && d2 < distToLight && abs(dot((p2 - u_Cuboids[i].Position), XAxis)) <= u_Cuboids[i].Scale.x / 2.0 && abs(dot((p2 - u_Cuboids[i].Position), YAxis)) <= u_Cuboids[i].Scale.y / 2.0;
...
{
    ObjectData data;
    if (objectID >= 0 && objectID < u_SphereCount)
    {
        data.Albedo = u_Spheres[objectID].Albedo;
        data.Roughness = u_Spheres[objectID].Roughness;
        data.Metallic = u_Spheres[objectID].Metallic;
        data.Opacity = u_Spheres[objectID].Opacity;
        data.IOR = u_Spheres[objectID].IOR;
    }
    else if (objectID >= 100 && objectID < 100 + u_CuboidCount)
    {
        int cuboidIndex = objectID - 100;
        data.Albedo = u_Cuboids[cuboidIndex].Albedo;
        data.Roughness = u_Cuboids[cuboidIndex].Roughness;
        data.Metallic = u_Cuboids[cuboidIndex].Metallic;
        data.Opacity = u_Cuboids[cuboidIndex].Opacity;
        data.IOR = u_Cuboids[cuboidIndex].IOR;
    }
    else
    {
        // Default values for non-object hits (floor, light, sky)
    ...
    for (int sampleIdx = 0; sampleIdx < u_SampleCount; sampleIdx++)
    {
        ...
        for (int bounce = 0; bounce < u_BounceCount; bounce++) 
        {
            ...
            if (hit.ObjectID >= 0) // We hit a Sphere!
            {
                ...
                float alpha = GetObjectData(hit.ObjectID).Opacity; 
                float metallic = GetObjectData(hit.ObjectID).Metallic; // Use Metallic for reflections!
                ...
                vec3 diffuseColor = GetObjectData(hit.ObjectID).Albedo * finalIntensity;
                ...
                vec3 specularTint = mix(vec3(1.0), GetObjectData(hit.ObjectID).Albedo, metallic);
                ...
                if (GetObjectData(hit.ObjectID).Metallic <= 0.01) break;

                // --- 3. Prepare for Next Bounce ---
                if (alpha < randomFloat(rngState)) // It is a transparent/glass object!
                {
                    ...
                    float refractionRatio = isInside ? GetObjectData(hit.ObjectID).IOR : (1.0 / GetObjectData(hit.ObjectID).IOR);
                    ...
                    throughput *= GetObjectData(hit.ObjectID).Albedo;
                }
                else 
                {
                    ...
                    float roughness = GetObjectData(hit.ObjectID).Roughness;
            ...
            else
            {
                ...
                vec3 skyColor = mix(vec3(0.05, 0.05, 0.05), vec3(0.73, 0.98, 1.0) * 0.05, gradientFactor);
